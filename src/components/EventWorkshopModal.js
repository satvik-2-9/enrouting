...
const EventWorkshopModal = (props) => {
  const { type, event, workshop, setEventModal, setWorkshopModal, locked, handleRegisterClick } = props;
  ...
  return (
    <div className="EventWorkshopModal">
      {submissionModal && <SubmissionModal event={event} setSubmissionModal={setSubmissionModal} />}
      {resultsModal && <ResultsModal event={event} setResultsModal={setResultsModal} />}
        ...
        {type === 'event' ? (
          <div className="EventWorkshopModal-container">
            <div className="EventWorkshopModal-title-row">
              <h1>{event.topic}</h1>
              {locked && <button onClick={() => handleRegisterClick()}>Register for event</button>}
            </div>
            {!locked ? (
              ...
              <div className="registration-details-div">
                <p className="registration-text">Registration fee: ₹{event.soloPrice} (for single) and ₹{event.groupPrice} (for group)</p>
              </div>
            )}
            <div
            ...
            {!locked && (
              <div className="EventWorkshopModal-button-container">
                <button onClick={() => setSubmissionModal(true)}>Make submissions</button>
                <button onClick={() => setResultsModal(true)}>View Results</button>
              </div>
            )}
          </div>
        ) : (
